...
console.log(fenomenler[3].followers)
...
function indekseGoreFenomen(birDizi, birSayi){
  const fenomen = fenomenler[birSayi];
 return `${birSayi}. indekste bulunan fenomen: ${fenomen.profile}`;
}
...
const yeniFenomenler = fenomenSil(fenomenler, 0);
...
function fenomenEkle(fenomenler,number,profile,followers,posts,platform) {
  ...
  yeniDizi.push(yeniFenomen);
  return yeniDizi

}

const fenomenEkleSonuc = fenomenEkle(fenomenler, 6, "Workintech", 10000000, 2022, "Instagram");
console.log(fenomenEkleSonuc);
...
function enFenomenler(fenomenler) {
  const enFenomenlerListesi = [];
  for (let i = 0; i < fenomenler.length; i++) {
    const fenomen = fenomenler[i];
    if (fenomen.followers > 100000000) {
      enFenomenlerListesi.push(fenomen.profile);
    }
  }
  return enFenomenlerListesi;
}

const enFenomenlerListesi = enFenomenler(fenomenler);
console.log(enFenomenlerListesi);
...
function fenomenGonderimSayisi(fenomenler,profile){
  ...
  const gonderimSayisi = fenomenGonderimSayisi(fenomenler, 'Will Smith');
  console.log(gonderimSayisi);
...
function platformaGoreCokGonderiYapanFenomen(fenomenler,platform){
  ...
  for (let i = 0; i < fenomenler.length; i++) {
    const fenomen = fenomenler[i];
    if (fenomen.platform === platform && fenomen.posts !== "NA" && fenomen.posts > enCokGonderiSayisi) {
...
const enCokGonderiYapan = platformaGoreCokGonderiYapanFenomen(fenomenler, 'TikTok');
console.log(enCokGonderiYapan);
